...
class ValidatorBroadcastBase:
    ...
    def verify_broadcast_sender(
        ...
        mothership_hotkey = ValiConfig.MOTHERSHIP_HOTKEY_TESTNET if self.is_testnet else ValiConfig.MOTHERSHIP_HOTKEY
        if not mothership_hotkey:
            bt.logging.warning(f"[SECURITY] MOTHERSHIP_HOTKEY not configured. Cannot verify {broadcast_name}.")
            return False

        if sender_hotkey != mothership_hotkey:
            bt.logging.warning(
                f"[SECURITY] Rejected {broadcast_name} from {sender_hotkey}. "
                f"Expected {mothership_hotkey}."
            )
            return False
