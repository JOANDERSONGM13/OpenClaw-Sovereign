...
class ValiBkpUtils:
    ...
    @staticmethod
    def rebuild_accounts_on_cpt_update(running_unit_tests=False) -> bool:
        """
        Flag file to indicate if accounts need to be rebuilt for the CPT update (2026).
        """
        suffix = "/tests" if running_unit_tests else ""
        file_path = ValiConfig.BASE_DIR + f"{suffix}/validation/rebuild_accounts_cpt_2026.json"
        
        if os.path.exists(file_path):
            return False
            
        # Create flag file to mark as done (after successful migration)
        # Note: The caller is responsible for creating this file after success.
        # This method just checks if it exists.
        return True

    @staticmethod
    def mark_cpt_migration_complete(running_unit_tests=False):
        suffix = "/tests" if running_unit_tests else ""
        file_path = ValiConfig.BASE_DIR + f"{suffix}/validation/rebuild_accounts_cpt_2026.json"
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        with open(file_path, 'w') as f:
            json.dump({"timestamp": TimeUtil.now_in_millis(), "status": "completed"}, f)
